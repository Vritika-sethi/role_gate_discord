...
const StepCard = ({ children, isComplete, icon }: { children: ReactNode; isComplete: boolean; icon: ReactNode; }) => (
  <motion.div
    ...
    className={`relative overflow-hidden rounded-2xl border p-5 shadow-lg transition-all duration-500 ${
      isComplete
...
const RoleItem = ({ role, isClaimed, claimingRole, onGrant }: { role: QualifiedRole; isClaimed: boolean; claimingRole: string | null; onGrant: (roleId: string) => void; }) => (
  <motion.div
    ...
    animate={{ opacity: 1, y: 0 }}
    className={`flex items-center justify-between rounded-lg p-3 shadow-inner transition-all duration-300 ${
      isClaimed ? "bg-green-900/40" : "bg-gray-700/50"
    }`}
  >
    <div>
    ...
    <button
      ...
      className={`rounded-md px-4 py-1.5 text-sm font-bold text-white shadow-sm transition-all duration-200 ${
        isClaimed ? "bg-gray-600 cursor-not-allowed" : "bg-green-600 hover:bg-green-500 hover:shadow-lg hover:shadow-green-500/20"
      }`}
    ...
    <main className="flex min-h-screen flex-col items-center justify-center p-4 sm:p-6 bg-gray-950 text-white">
      <div className="w-full max-w-md space-y-6">
        ...
        <div className="space-y-4">
          <StepCard isComplete={isStep1Complete} icon={<WalletIcon />}>
          ...
          <AnimatePresence>
            {isStep1Complete && (
              <StepCard isComplete={isStep2Complete} icon={<UserIcon />}>
                ...
                {!user.loggedIn ? (
                  <ActionButton onClick={siweLogin} className="bg-blue-600 hover:bg-blue-500">Sign In</ActionButton>
                ) : (
          ...
          <AnimatePresence>
            {isStep2Complete && (
              <StepCard isComplete={isStep3Complete} icon={<LinkIcon />}>
                ...
                <div className="space-y-2.5">
                  ...
                  {discordLinked && !walletLinked && (
                    <ActionButton onClick={linkWallet} className="bg-teal-600 hover:bg-teal-500">
          ...
          <AnimatePresence>
            {isStep3Complete && (
              <StepCard isComplete={isStep4Complete} icon={<CheckIcon />}>
                ...
                <ActionButton onClick={checkRoles} className="bg-purple-600 hover:bg-purple-500">
